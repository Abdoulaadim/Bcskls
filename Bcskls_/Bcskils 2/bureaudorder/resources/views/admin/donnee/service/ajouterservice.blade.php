<x-app-layout>
    ...
    <section class="max-w-4xl p-6 mx-auto bg-gray-800 rounded-md shadow-md dark:bg-gray-800 mt-8">
        <h1 class="text-xl font-bold text-white capitalize dark:text-white text-center mb-14"> <u>Ajouter Service </u></h1>
        <form method="post" action="{{ route('ajouterservice') }}">
            @csrf
            <div class="grid xl:grid-cols-2 gap-6 mt-4 sm:grid-cols-1">
                ...
                <div>
                    <label class="text-white dark:text-gray-200" for="usernaservice">Service: </label>
                    <input id="nomservice" name="nomservice" type="text" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-300 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-500 dark:focus:border-blue-500 focus:outline-none focus:ring">
                </div>
